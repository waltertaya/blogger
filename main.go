...
func main() {
	...
	mux.Handle("/api/v1/users/follow", middlewares.AuthMiddleware(http.HandlerFunc(handlers.FollowUserHandler)))
	mux.Handle("/api/v1/users/unfollow", middlewares.AuthMiddleware(http.HandlerFunc(handlers.UnfollowUserHandler)))
	mux.Handle("/api/v1/users/following", middlewares.AuthMiddleware(http.HandlerFunc(handlers.GetFollowingUsersHandler)))
	...
	mux.HandleFunc("/api/v1/blogs/search", handlers.SearchBlogsHandler)
	...
	mux.HandleFunc("/api/v1/blogs/comments", handlers.GetBlogCommentsHandler)
	mux.Handle("/api/v1/blogs/following", middlewares.AuthMiddleware(http.HandlerFunc(handlers.GetFollowingBlogsHandler)))
